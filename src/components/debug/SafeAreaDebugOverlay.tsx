...
export const SafeAreaDebugOverlay = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (url.searchParams.get("debugSafeArea") === "1") {
        localStorage.setItem("nb_debug_safe_area", "1");
      }
      setEnabled(localStorage.getItem("nb_debug_safe_area") === "1");
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    (window as any).__nbSnapshotSafeArea = snap;
    if (enabled) snap();
    return () => {
      try { delete (window as any).__nbSnapshotSafeArea; } catch {}
    };
  }, [enabled, insets]);
