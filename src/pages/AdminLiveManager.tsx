...
const AdminLiveManager = () => {
  ...
  const handleSubmit = async () => {
    ...
    let error: any;
    if (editingId) {
      ({ error } = await (supabase as any).from("live_sessions").update(payload).eq("id", editingId));
    } else {
      ({ error } = await (supabase as any).from("live_sessions").insert({ ...payload, created_by: user?.id, is_active: false }));
    }
    if (error) {
